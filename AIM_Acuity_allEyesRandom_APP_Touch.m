...
%% Version 28/1/2022
function [output_graph, acuity, which_eye, correction,trialRecordData] = ...
    AIM_Acuity_allEyesRandom_APP_Touch(view_distance, Patient_Name, number_of_Trials, correction_type)
...
% number_of_Trials=2; correction_type = 1


fprintf("#######################AIM_Acuity_Randomised_Eye_Starting_%s#######################\n", datestr(now,'mm-dd-yyyy HH-MM-SS'))
examtime= datetime ;
...
%% Hard-coding testing parameters
...
% scrnWidthCm=70; % screen width (cm)
...
cSize=5;% hard coded for Nicos experiment, force it to make the right cell size regardless of GUI
...
try
    for i=1:length(randomeye)%run through 3 eye conditions
        ...
        dataFile=fullfile(cd,'Patient Data', sprintf('%s_AIM_Acuity.mat', testSName));  % file path to unique Mat fes
        ...
        instcommand = audioread(soundFile); % sound file when mouse is clicked
        sound(instcommand, 22000)
        WaitSecs(5)
        ...
        for trialNo=1:nTrials % work through all trials
            ...
            for contNum=1:nContrasts % work though the list of test contrasts
                ...
                %                 [~, ~, textBox] = DrawFormattedText(windowPtr,  'Click on the gap orientation of each C', 'center', 100, 0); % give instructions to subject
        ...
        output_graph(temp_var)= figure('visible','off');
        ...
        [angleFitobject,~,angThreshAcuity,AIMci,~,AIMcompared]=fitPFuncOriErr_Aim_AcuityRandomised(testLevel',abs(respErr)',90,1,testEye,sName); % fit cumulative Gaussian to the ori error data

        acuity{i}= AIMcompared;
    ...
    disp('IMPORTANT:Use only near AIM acuity < 100cm with additional HD screen!')
    expDuration=toc(expStart); % stop timer - how long was experiment?
...
function [ fitobject, gof, thresholdEst, ci, chiSqFitReject, AIMcompared] = fitPFuncOriErr_Aim_AcuityRandomised( tOri, mErr, ranErr, graphData,eye,sName, criterion,AIMtype  )
